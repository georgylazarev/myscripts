...
backup_log="$backupdir/logs/${root_date}_backup_log_before" # Файл логов
...
do
  ...
  sudo -u zimbra $zmbox -z -m $mbox getRestURL "/?fmt=tgz&query=before:$before_date" > $userdir/$mbox-${root_date}_before.tgz && echo "$(date +%T): Бэкап писем $mbox готов" >> $backup_log
  echo "" >> $backup_log
done
