...
after_date=$(date +%m/%d/%y -d "$root_date 2 days ago"); # Бэкап после этой даты
before_date=$(date +%m/%d/%y -d "$root_date"); # Бэкап до этой даты

echo "$(date +%T): Начало бэкапирования" > $backup_log;
echo "" >> $backup_log;

echo "$(date +%T): Постоение списка активных аккаунтов" >> $backup_log;
sudo -u zimbra $zmprov -l sa "(zimbraAccountStatus=active)" > $active_accounts && echo "$(date +%T): Список построен" >> $backup_log;
echo "" >> $backup_log;
...
do
  ...
  if ! [ -d $backupdir/backups/$mbox/ ]; then
    ...
    mkdir $backupdir/backups/$mbox && echo "$(date +%T): Директория $mbox создана" >> $backup_log;
  fi

  if ! [ -d $backupdir/backups/$mbox/$cur_year ]; then
    echo "$(date +%T): Директория $mbox/$cur_year отсутствует" >> $backup_log;
    mkdir $backupdir/backups/$mbox/$cur_year && echo "$(date +%T): Директория $mbox/$cur_year создана" >> $backup_log;
  fi

  if ! [ -d $backupdir/backups/$mbox/$cur_year/$cur_month ]; then
    echo "$(date +%T): Директория $mbox/$cur_year/$cur_month отсутствует" >> $backup_log;
    mkdir $backupdir/backups/$mbox/$cur_year/$cur_month && echo "$(date +%T): Директория $mbox/$cur_year/$cur_month создана" >> $backup_log;
  fi
  ...
  if ! [ -d $backupdir/backups/$mbox/$cur_year/$cur_month/$cur_day ]; then
    ...
    mkdir $backupdir/backups/$mbox/$cur_year/$cur_month/$cur_day && echo "$(date +%T): Директория $mbox/$cur_year/$cur_month/$cur_day создана" >> $backup_log;
  fi

  userdir="$backupdir/backups/$(echo $mbox | sed 's/@/\@/')/$cur_year/$cur_month/$cur_day";
  sudo -u zimbra $zmbox -z -m $mbox getRestURL "/?fmt=tgz&query=after:$after_date and before:$before_date" > $userdir/$mbox-$root_date.tgz && echo "$(date +%T): Бэкап писем $mbox готов" >> $backup_log;
  echo "" >> $backup_log;
done
